// jQuery and Node.js code at the same place

$(function(){

	// Showing some node stuffs
	...
	$.get('http://fresh-science.com/feed/', function(response){
		...
		rss.find('item').each(function(){
			...
			var content = item.find('description').html().split('</p>')[1]+'</p>';
			var urlRegex = /(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/g;

			// Fetch the first image of the article
			var imageSource = content.match(urlRegex)[1];
			//var imageSource = "trlololo";
